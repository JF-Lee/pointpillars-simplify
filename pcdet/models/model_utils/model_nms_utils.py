...
def class_agnostic_nms(box_scores, box_preds, nms_config, score_thresh=None):
    ...
    if score_thresh is not None:  # filter
    ...
    if box_scores.shape[0] > 0:
        ...
        # 返回被保留下的box的索引，selected_scores = None
        # selected_scores = None
        keep_idx, selected_scores = getattr(iou3d_nms_utils, nms_config.NMS_TYPE)(
                boxes_for_nms[:, 0:7], box_scores_nms, nms_config.NMS_THRESH, **nms_config
        )
        with open('keep_idx.txt', 'w') as f:
            for data in keep_idx.detach().cpu().numpy().flatten():
                f.write(str(data) + '\n')
...
def multi_classes_nms(cls_scores, box_preds, nms_config, score_thresh=None):
    ...
    for k in range(cls_scores.shape[1]):
        ...
        selected = []
        if box_scores.shape[0] > 0:
            box_scores_nms, indices = torch.topk(box_scores, k=min(nms_config.NMS_PRE_MAXSIZE, box_scores.shape[0]))  # sort
