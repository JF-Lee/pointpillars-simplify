...
class AnchorHeadTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, class_names, grid_size, point_cloud_range, predict_boxes_when_training):
        ...
        #self.target_assigner = self.get_target_assigner(anchor_target_cfg)  ### just use in training
    ...
    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, dir_cls_preds=None):
        ...
        num_anchors = anchors.view(-1, anchors.shape[-1]).shape[0]  # anchor 数量
        batch_anchors = anchors.view(1, -1, anchors.shape[-1]).repeat(batch_size, 1, 1)
        # 预测结果展开为一维 (batch_size, Z*Y*X*num_of_anchor_scale*anchor_rot, 3)
        ...
        # 对7个预测的box参数进行解码操作
        batch_box_preds = self.box_coder.decode_torch(batch_box_preds, batch_anchors)   ####### decode

        # anchor方向预测
        if dir_cls_preds is not None:
            dir_offset = self.model_cfg.DIR_OFFSET  # 0.78539 方向偏移
            dir_limit_offset = self.model_cfg.DIR_LIMIT_OFFSET  # 0

            # 方向预测结果展开为一维 (batch_size, Z*Y*X*num_of_anchor_scale*anchor_rot, 2)
            ...
            """
            重新将角度旋转回到激光雷达坐标系中，因此需要加回来之前减去的45度，
            如果dir_labels是1的话，说明方向在是180度的，因此需要将预测的角度信息加上180度，
            否则预测角度即是所得角度
            """
            batch_box_preds[..., 6] = dir_rot + dir_offset + period * dir_labels.to(batch_box_preds.dtype)

        if isinstance(self.box_coder, box_coder_utils.PreviousResidualDecoder):  # not use in pointpillars
